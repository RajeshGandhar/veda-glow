...
const envSchema = z.object({
  ...
  ORDER_ACCESS_TTL_MINUTES: z.coerce
    .number()
    .int()
    .min(5)
    .max(43200)
  ...
  WEBHOOK_QUEUE_CONCURRENCY: z.coerce
    .number()
    .int()
    .min(1)
    .max(500)
  ...
  WEBHOOK_RETRY_BACKOFF_MS: z.coerce
    .number()
    .int()
    .min(100)
    .max(60000)
    .default(2000),
  WEBHOOK_LOCK_LEASE_MS: z.coerce
    .number()
    .int()
    .min(5000)
    .max(900000)
  ...
  ALERT_STUCK_ORDERS_THRESHOLD: z.coerce
    .number()
    .int()
    .min(1)
    .max(100000)
    .default(10),
  ALERT_DLQ_GROWTH_THRESHOLD: z.coerce
    .number()
    .int()
    .min(1)
    .max(100000)
...
if (env.NODE_ENV === "production") {
  ...
  if (
    !env.ALLOW_LOCALHOST_IN_PROD &&
    (env.FRONTEND_ORIGIN.includes("localhost") ||
      env.FRONTEND_ORIGIN.includes("127.0.0.1") ||
      env.FRONTEND_ORIGIN.includes("5173"))
  ) {
    throw new Error(
      "FRONTEND_ORIGIN cannot contain localhost/dev URLs in production unless ALLOW_LOCALHOST_IN_PROD=true",
    );
  }
}
